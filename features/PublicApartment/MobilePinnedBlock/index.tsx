        ...
        <CardContent>
          <Stack direction={'row'} justifyContent={'center'} sx={{ mb: 2 }}>
            ...
            {checkInDate && checkOutDate && (
              <Box sx={{ ml: 2 }}>
                <span style={{ color: '#000000' }} onClick={() => setOpen(true)}>
                  {generateDateRangeString(checkInDate, checkOutDate)}
                </span>
              </Box>
            )}
          </Stack>
          ...
          {checkInDate && checkOutDate ? (
            <Button variant="contained" sx={{ width: '100%' }} size="large" onClick={handleClickOpen}>
              Связаться с хозяином
            </Button>
          ) : (
            <Button variant="contained" sx={{ width: '100%' }} size="large" onClick={() => setOpen(true)}>
              Выбрать даты
            </Button>
          )}
        </CardContent>
