...
export default function SyncAccountInfo({ parsedInfo }: { parsedInfo: any }) {
  const {
    smartName,
    profilePictureUrl,
    managedListings,
    ...
    <Box sx={{ mt: 10 }}>
      <Grid container spacing={{ xs: 2, md: 4 }} sx={{}}>
        ...
        <SyncAccountInfoRow title={'Статус профиля'}>
          {isSuperhost ? <Chip label="Суперхост" /> : <Chip label="НЕ Суперхост" />}
        ...
        <SyncAccountInfoRow title={'Рейтинг'}>
          <Stack direction={'row'} alignItems={'center'} sx={{ mb: 2 }}>
            <StarIcon fontSize={'large'} sx={{ color: '#FF5A5F', mr: 0.5 }} />
            <Typography variant="h5">64 отзыва</Typography>
          </Stack>
          ...
          <Stack direction={'row'} spacing={4}>
            ...
            <Stack spacing={1}>
              <Stack direction={'row'} alignItems={'center'}>
                <StarIcon fontSize={'small'} sx={{ color: '#FF5A5F', mr: 0.5 }} />
                <Typography variant="body1">{reviewsCountFromGuest} отзыва</Typography>
              </Stack>
              <Stack direction={'row'} alignItems={'center'}>
                <StarIcon fontSize={'small'} sx={{ color: '#FF5A5F', mr: 0.5 }} />
                <Typography variant="body1">{reviewsCountFromHost} отзыва</Typography>
              </Stack>
            </Stack>
          </Stack>
        </SyncAccountInfoRow>
