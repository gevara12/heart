...
export default function AccountInfo({ parsedInfo, ...rest}:{
	parsedInfo:any,
	[key: string]: any,
}) {
			...
			<Grid container spacing={{xs:2, md:4}} sx={{}}>
				{ smartName && (
						<AccountInfoRow title={'Имя'}>
							<Typography variant="body1" sx={{width:isMobile?'100%':552}}>{smartName}</Typography>
						</AccountInfoRow>
				)}
				{ about && (
						<AccountInfoRow title={'О себе'}>
							<Typography variant="body1" sx={{width:isMobile?'100%':552}}>{about}</Typography>
						</AccountInfoRow>
				)}
				{ profilePictureUrl && (
				...
				{ isSuperhost && (
						<AccountInfoRow title={'Статус профиля'}>
							<div><Chip label="Суперхост" color="secondary" variant="outlined" /></div>
							<Alert severity="info" sx={{mt:{xs:2,md:4}, display:"inline-flex"}}>Топ-сервис на HeartApart</Alert>
            </AccountInfoRow>
				)}
				<AccountInfoRow title={'Объявления'}>
					...
					<Alert severity="info" sx={{mt:{xs:2,md:4}}}>
						<Typography variant="body2">После подтверждения переноса данных ваши объявления сохранятся как черновики в профиле на HeartApart.</Typography>
					</Alert>
				</AccountInfoRow>
