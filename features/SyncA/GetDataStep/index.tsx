...
import styles from './GetDataStep.module.css';

const isLinkCorrect = (link) => {
  return (
    link.indexOf('airbnb.ru') === -1 &&
    link.indexOf('airbnb.com') === -1 &&
    (link.indexOf('/users/show') === -1 || (link.indexOf('/users') === -1 && link.indexOf('/listings') === -1))
  );
};
...
export default function GetDataStep() {
  ...
  const serviceLinkValidate = () => {
    ...
    if (isLinkCorrect(serviceLink)) {
      ...
      setLinkHasError(true);
      setLinkError(
        'Поле должно содержать ссылку вида "airbnb.*/users/show/999999999" или "airbnb.*/users/999999999/listings"',
      );
      return formIsValid;
    }
  ...
  return (
    <form onSubmit={handleSync}>
      <div className={styles.host}>
        <Typography variant="h4" sx={{ mt: 11 }}>
          Перенос данных с сервиса А
        </Typography>
        <Typography variant="body1" sx={{ mt: 4 }}>
          <b>Мы можем перенести из открытых данных следующую информацию (при ее наличии):</b>
        </Typography>

        <ParseInfoList />

        <Typography variant="body1" sx={{ mt: 4 }}>
          <b>Вставьте ссылку на ваш профиль, данные будут заполнены автоматически.</b>
        </Typography>

        <Alert severity="warning" sx={{ mt: 2.5 }}>
          Убедитесь, что у вас есть доступ к редактированию профиля!
        </Alert>
        ...
        <Typography variant="body1" sx={{ mt: 4 }}>
          Мы заботимся о доверии, поэтому нам важно подтвердить, что указанный по ссылке профиль на А действительно ваш.
        </Typography>

        <UserCodeRow />

        <Typography variant="body1" sx={{ mt: 3 }}>
          Зайдите в свой профиль на сервисе А, перейдите в режим редактирования и вставьте код в любое место в тексте
          раздела Информация (About). Сохраните изменения.
        </Typography>
        <Typography variant="body1" sx={{ mt: 3 }}>
          После этого вернитесь сюда и нажмите кнопку ниже “Продолжить”.
        </Typography>
        ...
        {requestError && (
          <Alert severity="error" sx={{ mt: 3 }}>
            ...
            <Typography variant="body2">
              Проверьте, что указали верный код в нужном поле и сохранили изменения в профиле А.
            </Typography>
          </Alert>
        )}

        <Stack direction="row" spacing={2} sx={{ mt: 4, mb: 17 }} justifyContent="stretch">
          <LoadingButton variant="contained" type="submit" loading={loading} disabled={loading} sx={{ px: 8 }}>
            Продолжить
          </LoadingButton>
        </Stack>
      </div>
