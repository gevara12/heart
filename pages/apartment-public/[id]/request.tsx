...
import { Container, Typography, Stack, Button, Box, Avatar, Grid, Divider, Tooltip } from '@mui/material';
...
export default function ApartmentRequest() {
    ...
    if (checkInDate && checkOutDate && currentUser) {
      setMessage(
        `Добрый день! Нашел вашу квартиру на heartapart.ru <br/>
                ...
                Хочу забронировать с ${format(new Date(checkInDate), 'd.MM.yyyy', { locale: ruLocale })} по ${format(
          new Date(checkOutDate),
          'd.MM.yyyy',
          { locale: ruLocale },
        )}, на 2 взрослых, без детей, без животных. <br/>
  ...
  return (
    <Layout isHero={true}>
      <SEO />
      <section>
        {Object.keys(currentApartment.publicInfo).length !== 0 && (
          <Container maxWidth="lg">
            <Button sx={{ mt: 6 }} onClick={() => router.push(`/apartment-public/${id}`)}>
              <ArrowBackIcon sx={{ mr: 1 }} />
              <div>Вернуться к объявлению</div>
            </Button>
                ...
                <Grid item xs={12} md={2}>
                  {isMobile && (
                    <Typography variant="h5" component="div">
                      Отправьте запрос {apartOwner?.name}
                    </Typography>
                  )}
                  <Avatar
                    sx={{
                ...
                <Grid item xs={12} md={6}>
                  ...
                  {isMobile ? (
                    ...
                    <Typography variant="body1" component="div" sx={{}}>
                      Чтобы хозяин смог быстрее рассмотреть ваш запрос, воспользуйтесь приведенным шаблоном. Скопируйте
                      текст и вставьте в сообщение в выбранном месседжере. Вы также можете задать вопросы по жилью,
                      которые не указаны в объявлении, написать пару слов о себе и цели поездки.
                    </Typography>
                  )}
                  ...
                  <Typography variant="h5" component="div" sx={{ mt: 6.5 }}>
                  ...
                  >
                    ...
                    <Box sx={{ ml: 2.5 }}>
                      <Typography variant="body1" component="div">
                        {socials.filter((social) => social.active)[0].name}
                      </Typography>
                      <Typography variant="body2" component="div">
                        Хозяин указал этот способ связи как предпочтительный
                      </Typography>
                    </Box>
                  </Stack>
                  <Stack direction={'row'} spacing={2} sx={{ mt: isMobile ? 2.5 : 4 }}>
                    {socials.map(
                      (social, i) => !social.active && <SocialRoundBox social={social} showTitle={true} key={i} />,
                    )}
                  </Stack>
                </Grid>
              </Grid>
