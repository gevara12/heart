      ...
      <Container maxWidth='lg'>
        <Stack spacing={isMobile?2:1}>
          <Stack direction={isMobile? 'column':'row'} spacing={0.5}>
            ...
            <a href={`mailto:${supportLink}`} target='_blank' rel='noopener noreferrer' style={{color:'#757575',textDecoration:'none'}}>
              <Typography variant='h5'>{supportLink}</Typography>
            </a>
          </Stack>
          <Stack direction={isMobile? 'column':'row'} spacing={0.5}>
            <Typography variant='h5'>Для обратной связи:</Typography>
            <a href={`mailto:${feedbackLink} `} target='_blank' rel='noopener noreferrer' style={{color:'#757575', textDecoration:'none'}}>
              <Typography variant='h5'>{feedbackLink}</Typography>
            </a>
          </Stack>
        </Stack>
        <Stack direction={isMobile? 'column':'row'} spacing={isMobile?2:0} justifyContent='space-between' sx={{ mt: 10 }}>
          <Stack direction='row' spacing={5}>
            {navigation.map((navLink,i)=>
              <Link href={navLink.url} passHref key={i}>
                <a style={{color:'#1C1B1F', textDecoration:'none'}}><Typography variant='body1'>{navLink.title}</Typography></a>
              </Link>
            )}
          </Stack>
